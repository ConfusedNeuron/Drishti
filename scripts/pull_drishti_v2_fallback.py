...
try:
    ...
    import sys
    print(f"ERROR: Could not import dependencies from pull_drishti_data.py.\nDetails: {e}\n"
          "Make sure you are running this in the correct Python environment where pandas and tqdm are installed.")
    sys.exit(1)
...
def discover_universe(session, with_fallbacks: bool = False) -> dict:
    ...
    for idx in indices:
        ...
        for dt in _snapshot_dates():
            ...
            if not rows:
                print(f"  [WARN] {idx} @ {dt} returned 0 rows (likely entitlement block).")
                print(f"  Falling back to current members only (INDX_MWEIGHT) for {idx}...")
                try:
                    rows = bds(session, idx, "INDX_MWEIGHT")
                except Exception as e:
                    print(f"  [WARN] Fallback {idx} failed: {e}")
                    continue
                hist_failed = True  # Stop querying historical dates for this index
                dt = date.today().strftime("%Y%m%d") # Tag with today's date

...
**READ FIRST: `docs/status-2026-07-07-handoff.md`** — the full handoff (Zerodha setup steps, deferred items, v5 candidates, process notes). Note its §1 merge checklist is now DONE — see the 2026-07-23 status banner at its top.
**Final presentation DELIVERED 2026-06-16 ✅** — market-risk + spillover narrative (see `presentation/`, gitignored).
**Dashboard audit DONE** (`docs/audit-2026-07-04-dashboard.md`) → **fix sprint MERGED** (PR #12) → **v4 wave 1 MERGED**: F1 Zerodha sync + P&L (PR #13), F2 Efficient Frontier Studio (PR #14), F4 Spillover Lab (PR #15) — the full stacked chain #12→#13→#14→#15 is **merged to `main`**; `main` has **298 tests passing**. v4 PRDs: `design/prd-2026-07-04-*.md`; feature design docs: `docs/superpowers/specs/2026-07-06-f{1,2,4}-*-design.md`. **NEXT: merge PR #16 (`chore/deprecation-warnings`) + the stacked `chore/review-fixes` branch, 5-min browser eyeball (Frontier tab + Spillover Lab + Holdings P&L), then pick v5 scope** (F3 watchlist / F5 horizon report / regime-conditioned frontier — see handoff §6). Read `docs/frontend/code.md` before any frontend work.
...
### Active branch
`chore/review-fixes` (stacked on `chore/deprecation-warnings`, which is open as **PR #16**). The entire v4 wave-1 stacked chain — `fix/audit-2026-07-04-sprint` (PR #12) → `feature/v4-f1-zerodha` (PR #13) → `feature/v4-f2-frontier` (PR #14) → `feature/v4-f4-spillover-lab` (PR #15) — **was merged to `main`**; those four branches are deleted locally (remote copies may linger). **`main`: 298 tests passing.** PR #16 adds deprecation-warning cleanup, a frontier axis fix, README screenshots, and a Render synthetic-cache build fix; `chore/review-fixes` (2026-07-23) adds review remediations — dead `main.py` removed, pyproject description, CORS tightened via `DRISHTI_CORS_ORIGINS` (default localhost), `logging.basicConfig` in `app.py`, shared `prepare_portfolio_inputs()` in `src/risk/returns.py` deduplicating the route/MCP returns-prep glue.
...
- ✅ 298 unit tests passing on `main` (v4 wave-1 chain PRs #12–#15 all merged)
...
├── tests/                           # pytest — 298 tests passing on `main`
